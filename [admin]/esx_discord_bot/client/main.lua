...
Citizen.CreateThread(function()
	while true do
		...
		if(IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then
			if not isIncarPolice and PlayerData.job.name ~= 'police' and PlayerData.job.name ~= 'nosse' and PlayerData.job.name ~= 'ambulance' and PlayerData.job.name ~= 'fib' and PlayerData.job.name ~= 'army' then
