...
AddEventHandler('sendToDiscord', function(name, message)
    sendToDiscord(name, message, Config.red)
end)
...
AddEventHandler('esx:playerconnected', function()
  if(settings.LogLoginServer) then
	...
	sendToDiscord(_U('server_connecting'), GetPlayerName(source) .." ".. _('user_connecting') .. "(" .. steamIdentifier .. ")",Config.grey)
  end
end)
...
AddEventHandler('playerDropped', function(reason)
  ...
  local player = source
    ...
    sendToDiscord(_U('server_disconnecting'), GetPlayerName(source) .." ".. _('user_disconnecting') .. "("..reason..")" .. "(" .. steamIdentifier .. ")",Config.grey)
  end
end)
...
AddEventHandler('esx:onPlayerDeath', function(data)
    ...
    if data.killedByPlayer then
		sendToDiscord(_U('server_kill'), GetPlayerName(data.victim) .." wurde von "..GetPlayerName(data.killerServerId).. " getötet",Config.red)
...
RegisterServerEvent("esx:search")
AddEventHandler("esx:search", function(gang)
	sendToDiscord("Durchsuchen", gang .. " durchsucht jemanden",Config.red)
end)

RegisterServerEvent("esx:webhook")
AddEventHandler("esx:webhook", function(title, message)
	sendToDiscord(title, message,Config.red)
end)

RegisterServerEvent("esx:support")
AddEventHandler("esx:support", function(name)
	sendToDiscord("Support", name .. " braucht IC Support",Config.red)
end)
